...
Public Class Veiculo
    ...
    Public Function ValidarExclusao(ByVal Veiculo As Veiculo)
        ...
        If Banco.BancoEmUso = "ORACLE" Then
            ...
            If Banco.Conexao.Execute(String.Format("SELECT COUNT(*) FROM SGIPA.TB_AG_CS WHERE AUTONUM_VEICULO = {0} AND AUTONUM_TRANSPORTADORA = {1}", Veiculo.ID, Veiculo.Transportadora.ID)).Fields(0).Value > 0 Then
                Return False
            End If

            If Banco.Conexao.Execute(String.Format("SELECT COUNT(*) FROM OPERADOR.TB_AG_RETIRADA WHERE AUTONUM_VEICULO = {0} AND AUTONUM_TRANSPORTADORA = {1}", Veiculo.ID, Veiculo.Transportadora.ID)).Fields(0).Value > 0 Then
                Return False
            End If
        ...
        Else
            ...
            If Banco.Conexao.Execute(String.Format("SELECT COUNT(*) FROM OPERADOR.DBO.TB_AG_RETIRADA WHERE AUTONUM_VEICULO = {0} AND AUTONUM_TRANSPORTADORA = {1}", Veiculo.ID, Veiculo.Transportadora.ID)).Fields(0).Value > 0 Then
                Return False
            End If
    ...
    Public Function ValidarVeiculosIguais(ByVal Veiculo As Veiculo, ByVal Edit As Boolean)
        ...
        If Banco.BancoEmUso = "ORACLE" Then
            If Edit Then
                If Banco.Conexao.Execute(String.Format("SELECT COUNT(*) FROM OPERADOR.TB_AG_VEICULOS WHERE PLACA_CAVALO='{0}' AND PLACA_CARRETA='{1}' AND ID_TRANSPORTADORA={2} AND AUTONUM <> {3}", Veiculo.Cavalo, Veiculo.Carreta, Veiculo.Transportadora.ID, Veiculo.ID)).Fields(0).Value = 0 Then
                    Return True
                End If
            Else
        ...
        Else
            If Edit Then
                If Banco.Conexao.Execute(String.Format("SELECT COUNT(*) FROM OPERADOR.DBO.TB_AG_VEICULOS WHERE PLACA_CAVALO='{0}' AND PLACA_CARRETA='{1}' AND ID_TRANSPORTADORA={2} AND AUTONUM <> {3}", Veiculo.Cavalo, Veiculo.Carreta, Veiculo.Transportadora.ID, Veiculo.ID)).Fields(0).Value = 0 Then
                    Return True
                End If
            Else
    ...
    Public Function ObterCarreta(ByVal Cavalo As String, ByVal ID_Transportadora As String) As String
        ...
        If Not Rst.EOF Then
            Return Rst.Fields("PLACA_CARRETA").Value.ToString()
        End If
