    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            If Request.QueryString("id") IsNot Nothing Then
                ...
                Dim SQL As String = "SELECT A.NUM_PROTOCOLO || '/' || A.ANO_PROTOCOLO AS PROTOCOLO, A.ID_CONTEINER, C.NUMERO AS BL FROM TB_CNTR_BL A INNER JOIN TB_AMR_CNTR_BL B ON A.AUTONUM = B.CNTR INNER JOIN TB_BL C ON B.BL = C.AUTONUM WHERE A.AUTONUM = " & Val(Request.QueryString("id").ToString()) & " AND A.AUTONUM_TRANSPORTE_AGENDA = " & Val(Session("SIS_ID").ToString())

                Dim Dt As New DataTable

                Using Connection As New OleDbConnection(Banco.StringConexao(True))
                    Using Adap As New OleDbDataAdapter(New OleDbCommand(SQL, Connection))
                        ...
                        If Dt.Rows.Count > 0 Then
                            ...
                            Dim Conteiner As String = Dt.Rows(0)("ID_CONTEINER").ToString()
                            ...
                            If Request.QueryString("edit") IsNot Nothing Then
                                Assunto = "Agendamento atualizado: " & Protocolo & "    Contêiner: " & Conteiner
                            Else
                                Assunto = "Envio de Documento: Agendamento " & Protocolo & "    Contêiner: " & Conteiner
                            End If

                            Dt = Documento.Consultar(Request.QueryString("id").ToString(), Request.QueryString("motorista").ToString(), Request.QueryString("veiculo").ToString(), Session("SIS_ID").ToString())
                            ...
                            If Dt.Rows.Count > 0 Then
                                ...
                                Email.EnviarEmail(
                                  ...
                                  "",
                                  "",
                                  Mensagem & "<br/><br/>Protocolo: " & Protocolo & " <br/>BL: " & BL & "<br/>Contêiner: " & Conteiner & "</p><br/>
                               <p>E-mail do cliente: " & Me.lblEmail.Text & "</p>")

                                Banco.Conexao.Execute("UPDATE TB_CNTR_BL SET MOTIVO_AGENDAMENTO_RECUSADO = NULL, FLAG_LIBERADO=0, USUARIO_LIBERACAO_AGENDAMENTO = NULL, DT_AGENDAMENTO_LIBERACAO = NULL WHERE AUTONUM = " & Val(Request.QueryString("id").ToString()) & " AND AUTONUM_TRANSPORTE_AGENDA = " & Val(Session("SIS_ID").ToString()))
