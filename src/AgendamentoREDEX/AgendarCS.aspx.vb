    ...
    Private Function ValidarSaldoRestante(ByVal Qtde As String) As Boolean
        ...
        If Not String.IsNullOrEmpty(Me.lblCodigoPeriodo.Text) And Val(Me.lblCodigoPeriodo.Text) > 0 Then
            If Me.lblCodigoBooking.Text > "" Then
                ...
                DsDes = Banco.List(sSql)
                SQL.AppendLine(" SELECT AUTONUM_GD_RESERVA ")
                SQL.AppendLine(" FROM (")
                SQL.AppendLine("  SELECT A.AUTONUM_GD_RESERVA, ")
                If DsDes.Rows.Count > 0 Then
                    ...
                    If Convert.ToInt32(DsDes.Rows(0)("FLAG_CS_VOLUME").ToString()) > 0 Then
                        SQL.AppendLine("  case when NVL(rl.limite,0)>0 then rl.limite else NVL(A.LIMITE_VOLUMES,0)   end   - (NVL(AG.QTD,0) +" & Qtde & ") LIMITE_VOLUMES,  ")
                    Else
                ...
                SQL.AppendLine("    INNER JOIN REDEX.TB_AGENDAMENTO_WEB_CS ON TB_AGENDAMENTO_WEB_CS_NF.AUTONUM_AGENDAMENTO = TB_AGENDAMENTO_WEB_CS.AUTONUM ")
                If Contar > 0 Then
                ...
                If Me.lblCodigoVeiculo.Text > "0" Then
                    SQL.AppendLine(" Select Case when max(NVL(cm.qtde,0))=0 then SUM(1)+1 else ")
                    SQL.AppendLine(" 0 End QTD, cs.AUTONUM_GD_RESERVA  ")
                    SQL.AppendLine("  FROM (select  distinct AUTONUM_GD_RESERVA , AUTONUM_VEICULO  from  redex.TB_AGENDAMENTO_WEB_CS where    AUTONUM_GD_RESERVA = " & Me.lblCodigoPeriodo.Text & " And autonum <> " & Val(Me.lblCodigoAgendamento.Text) & ") cs ")
                    SQL.AppendLine(" Left Join( select distinct 1 qtde , AUTONUM_GD_RESERVA from ")
                    SQL.AppendLine(" redex.TB_AGENDAMENTO_WEB_CS where AUTONUM_GD_RESERVA=" & Me.lblCodigoPeriodo.Text & " AND ")
                    SQL.AppendLine(" AUTONUM_VEICULO = " & Me.lblCodigoVeiculo.Text & " AND autonum <>" & Val(Me.lblCodigoAgendamento.Text) & ")  cm ")
                    SQL.AppendLine(" On cs.AUTONUM_GD_RESERVA=cm.AUTONUM_GD_RESERVA ")
                    SQL.AppendLine(" Group BY CS.AUTONUM_GD_RESERVA   ")
                Else
                    SQL.AppendLine("    Select NVL(SUM(1),0)+1 QTD, AUTONUM_GD_RESERVA")
                    SQL.AppendLine("    FROM REDEX.TB_AGENDAMENTO_WEB_CS Where ")
                    SQL.AppendLine("    AUTONUM_GD_RESERVA=" & Me.lblCodigoPeriodo.Text & " And  autonum <>" & Val(Me.lblCodigoAgendamento.Text) & "")
                    SQL.AppendLine("    Group BY CS.AUTONUM_GD_RESERVA   ")
                End If
                SQL.AppendLine("  ) CAM On A.AUTONUM_GD_RESERVA = CAM.AUTONUM_GD_RESERVA")
                SQL.AppendLine("  WHERE A.SERVICO_GATE = 'A' AND A.TIPO = 'CS' AND A.PERIODO_INICIAL >= SYSDATE")
                ...
                SQL.AppendLine(" AND AUTONUM_GD_RESERVA=" & Me.lblCodigoPeriodo.Text)
                Ds = Banco.List(SQL.ToString())
