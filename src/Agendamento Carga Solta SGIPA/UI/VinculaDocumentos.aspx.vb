...
Public Class VinculaDocumentos
    ...
    Private Sub CarregarDocumentosAnexados()
        ...
        Me.gdvDocumentos.DataSource = Documento.Consultar(ObterMotoristaAgendamento(), ObterVeiculoAgendamento(), ObterIdAgendamento(), ObterLote(), Session("SIS_ID").ToString())
    ...
    Protected Sub gdvDocumentos_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gdvDocumentos.RowCommand
        ...
        If e.CommandName.Equals("DEL") Then
            ...
            Dim Liberado As String = Banco.Conexao.Execute("SELECT NVL(FLAG_LIBERADO,0) FLAG_LIBERADO FROM TB_AG_CS WHERE AUTONUM = " & ObterIdAgendamento()).Fields(0).Value.ToString()
            Dim Status As String = String.Empty

            Status = Banco.Conexao.Execute("SELECT NVL(IMPRESSO,0) IMPRESSO FROM TB_AG_CS WHERE AUTONUM = " & ObterIdAgendamento()).Fields(0).Value.ToString()
            ...
            If Documento.DeletarDocumento(IDAvImagem) Then
                CarregarDocumentosAnexados()
            End If
    ...
    Protected Sub chkAceite_CheckedChanged(sender As Object, e As EventArgs) Handles chkAceite.CheckedChanged
        Me.btnAnexar.Enabled = chkAceite.Checked
        Me.cbDocumentos.Enabled = chkAceite.Checked
        Me.txtUpload.Enabled = chkAceite.Checked
    End Sub
End Class
